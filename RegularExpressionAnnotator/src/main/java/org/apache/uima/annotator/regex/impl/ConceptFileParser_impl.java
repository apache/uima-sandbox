  ...
  public Concept[] parseConceptFile(File conceptFile) throws ResourceInitializationException {
    ...
    try {
      ...
      throw new ResourceInitializationException(RegExAnnotator.MESSAGE_DIGEST,
              "regex_annotator_error_parsing_rule_set_file", new Object[] { conceptFile
                      .getAbsolutePath() }, ex);
    }
    ...
    if (!isValid) {
      ...
      throw new ResourceInitializationException(RegExAnnotator.MESSAGE_DIGEST,
              "regex_annotator_error_xml_validation", new Object[] { conceptFile.getAbsolutePath(),
    ...
    noNamespace.ConceptDocument.Concept[] concepts = conceptSet.getConceptArray();
    for (int i = 0; i < concepts.length; i++) {
      // get concept meta data
      String conceptName = concepts[i].getName();

      // create new concept object
      org.apache.uima.annotator.regex.Concept concept = new Concept_impl(conceptName);
      ...
      for (int r = 0; r < ruleList.length; r++) {
        ...
        // create new rule
        Rule rule = new Rule_impl(regex, matchStrategy, matchType, id, confidence);
        ...
        if (ruleList[r].getMatchTypeFilter() != null) {
          ...
          noNamespace.FeatureDocument.Feature[] filterFeatures = ruleList[r].getMatchTypeFilter()
                  .getFeatureArray();
          for (int x = 0; x < filterFeatures.length; x++) {
            ...
            FilterFeature filterFeature = new FilterFeature_impl(featureName, featureValue);
        ...
        if (ruleList[r].getUpdateMatchTypeAnnotation() != null) {
          // iterate over all match type annotation update features and add them to the rule
          SetFeature[] updateFeatures = ruleList[r].getUpdateMatchTypeAnnotation()
                  .getSetFeatureArray();
          for (int x = 0; x < updateFeatures.length; x++) {
            ...
            Feature updateFeature = new Feature_impl(featureType, featureName, featureValue);
        ...
        if (ruleList[r].getRuleExceptions() != null) {
          // iterate over all match type annotation update features and add them to the rule
          noNamespace.ExceptionDocument.Exception[] exceptions = ruleList[r].getRuleExceptions()
                  .getExceptionArray();
      ...
      noNamespace.AnnotationDocument.Annotation[] annotationList = annotations.getAnnotationArray();
