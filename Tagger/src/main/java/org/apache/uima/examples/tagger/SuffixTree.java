...
public class SuffixTree {
   ...
   * int current_suffix_len){ int edge_count = 0; Set alphabet = new HashSet<Character>(); char []
   ...
   * 
   * Iterator<Entry> kv = edges.entrySet().iterator(); for (int u= 0; u<edges.size();u++){
   * 
   * Map.Entry m = (Map.Entry) kv.next(); Object key = m.getKey(); Edge e = (Edge) m.getValue();
   * 
   * if (nodes.get(e.end_node-1).suffix_node == -1){ System.out.println(key);
   * 
   * System.out.println("suffix_node "+nodes.get(e.end_node-1).suffix_node);
   * System.out.println("test "+chars[e.first_char_index]);
   * 
   * System.out.println(test_str.subSequence(e.first_char_index,e.last_char_index+1));
   *  } }
   *  }
   */
}
